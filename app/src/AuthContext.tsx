...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const performLogin = async (login: string, password: string, navigate: (path: string) => void) => {
    // Bestimmen, ob login eine E-Mail oder ein Benutzername ist
    const isEmail = login.includes('@');
    const queryParam = isEmail ? `email=${encodeURIComponent(login)}` : `username=${encodeURIComponent(login)}`;

    const response = await fetch(`http://localhost:3000/users?${queryParam}&password=${encodeURIComponent(password)}`);
